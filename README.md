...
- Logical brightness is 74% from 06:00 through 21:00 and 15% from 21:01 through 05:59. The LED driver limits full logical output to 10% electrical RGB output. All logical output is physically rotated 90 degrees counterclockwise on the ring; logical index 0 remains the 12 o'clock reference in firmware.
- In the normal clock page, logical index 12 is reserved as a breathing status indicator: green for working mode, cyan while time calibration is running, and latched red after a network/time-sync or LED refresh failure. Boot, provisioning, and sync status pages keep their existing full-ring effects. The ASRPRO UART module (offline voice control and TF-card audio playback) is optional; initialization/report failures are logged and do not affect the clock.
