...
- In the normal clock page, logical index 12 is reserved as a breathing status indicator: green for working mode, cyan while time calibration is running, and latched red after a network/time-sync or LED refresh failure. Boot, provisioning, and sync status pages keep their existing full-ring effects. LU6288 UART audio is optional; initialization/report failures are logged and do not affect the clock.
